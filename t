...
spawn(function()
    while wait(Sec) do
        if _G.Level then
            -- 🔥 Atualiza as informações da missão antes de iniciar o farm
            pcall(QuestCheck)
    ...
    -- Espera chegar e garantir que o NPC existe
    local tries = 0
    repeat
        task.wait(0.5)
        tries += 1
    until (Root.Position - QuestNeta()[6].Position).Magnitude <= 5 or tries >= 10

    -- Procura o NPC no workspace
    local npc = workspace:FindFirstChild(QuestNeta()[3]) or workspace.Map:FindFirstChild(QuestNeta()[3])

    -- Se o NPC existir, tenta pegar a missão até conseguir
    if npc then
        ...
        if not gotQuest then
            warn("⚠️ Falha ao pegar missão. Tentando novamente...")
        end
    else
        warn("❌ NPC da missão não encontrado:", QuestNeta()[3])
    end
end
